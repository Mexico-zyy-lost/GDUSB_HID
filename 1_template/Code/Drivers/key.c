...
{
    return gpio_input_bit_get(COL_KEY_PORT[key], COL_KEY_PIN[key]);
}

// col输出高/低电平，检测raw电平
void KEY_MATRIX_Scan(uint16_t *data,uint16_t *isFnPress)
{
	static uint8_t row = 0,col = 0,index = 0;
	uint8_t status[COL] = {2,2,2,2,2,2,2,2};
	uint16_t num = 0;
	...
	{
		status[col] = key_state_get(col);
		num += status[col];
	}
	// 若有低电平
	if(num < COL)
	{
		uint8_t totalNum = COL - num;
		uint8_t goal_Col[COL] = {};
		uint8_t temp_index = 0;
		for(uint16_t i = 0; i < COL;i++)
		{
			if(status[i] == 0)
			{
				goal_Col[temp_index] = i;
				temp_index++;
			}			
		}
		...
		{ 
			...
			// 按列再来一次
			for(col = 0; col < totalNum; col++)
			{
				uint8_t col_t = goal_Col[col];
				if(key_state_get(col_t))
				{
					if(key_filter(col_t))
					{
						uint16_t key_value = keyMap[row][col_t];//(row << 8) + col;
						*(data + index) = key_value;
						index++;
						isFnPress++;
					}
				}
...
{
    uint8_t new_state = key_state_get(key_id);    // 读取按键状态
